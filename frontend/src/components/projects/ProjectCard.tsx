...
export default function ProjectCard({ project, index = 0 }: ProjectCardProps) {
  ...
  const hiddenOnMobileCount = isTechExpanded
    ? 0
    : Math.max(0, Math.min(techStack.length, DESKTOP_TECH_LIMIT) - MOBILE_TECH_LIMIT);
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  {visibleTech.map((tech, techIndex) => (
                    <span
                      key={tech}
                      className={[
                        "rounded-full bg-white/10 backdrop-blur-sm px-2.5 py-1",
                        "text-[9px] sm:text-[10px] md:text-[11px] font-medium text-white/80",
                        "border border-white/20 transition-all duration-300",
                        "hover:bg-white/20 hover:text-white hover:border-white/30 hover:scale-105 hover:-translate-y-0.5",
                        // Hide items beyond mobile limit on small screens
                        techIndex >= MOBILE_TECH_LIMIT ? "hidden md:inline-flex" : "inline-flex",
                      ].join(" ")}
                  ...
                  {/* MOBILE: show remaining count beyond 3 */}
                  {!isTechExpanded && mobileRemainingCount > 0 && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setIsTechExpanded(true);
                      }}
                      className="md:hidden rounded-full bg-white/5 backdrop-blur-sm px-2.5 py-1 text-[9px] sm:text-[10px] font-medium text-white/70 border border-white/10 transition-all duration-300 hover:bg-white/15 hover:text-white hover:border-white/20"
                    >
                      +{mobileRemainingCount} more
                    </button>
                  )}

                  {/* DESKTOP: show remaining count beyond 5 */}
                  {desktopRemainingCount > 0 && (
                    <button
                      ...
                      className="hidden md:inline-flex rounded-full bg-white/5 backdrop-blur-sm px-2.5 py-1 text-[10px] md:text-[11px] font-medium text-white/70 border border-white/10 transition-all duration-300 hover:bg-white/15 hover:text-white hover:border-white/20"
                    >
                      +{desktopRemainingCount} more
                    </button>
                  )}

                  {/* SHOW LESS — appears on both after expanding */}
                  {isTechExpanded && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className="rounded-full bg-white/5 backdrop-blur-sm px-2.5 py-1 text-[9px] sm:text-[10px] md:text-[11px] font-medium text-white/70 border border-white/10 transition-all duration-300 hover:bg-white/15 hover:text-white hover:border-white/20"
                    >
                      Show less
                    </button>
                  )}
